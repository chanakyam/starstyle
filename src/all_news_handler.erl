...
welcome(Req, State) ->
	...
	% Url_all_news = string:concat("http://api.contentapi.ws/news?channel=entertainment_film&limit=10&format=short&skip=", integer_to_list(SkipItems)),
	% Url_all_news = "http://api.contentapi.ws/news?channel=entertainment_film&limit=10&format=short&skip=0",
	Url_all_news = "http://contentapi.ws:5984/contentapi_text_maxcdn/_design/yb_entertainment_film/_view/by_id_title_desc_thumb_date?descending=true&limit=10&skip=0",
	% io:format("all news : ~p~n",[MusicParams]),
	...
	{ok, Body} = all_news_paginated_dtl:render([{<<"videoParam">>,Params},{<<"allnews">>,ResAllNews},{<<"videos">>,VideoParams},{<<"latestvideos">>,LatestVideoParams},{<<"mostpopular">>,MostPopularParams},{<<"music">>,MusicParams}]),
    {Body, Req, State}.
